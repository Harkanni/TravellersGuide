...
// 	  console.log(success)
// 	  {latitude, longitude} = success.coords
// 	})
...
// 	fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=6.5227&lon=3.6218&exclude={part}&appid=a1064ec8f051ec43d04e8d0e055bbec3`)
//		  .then(res=>res.json())
...
/* async function getWeatherData () {
	...
	// FETCH DATA FROM OPEN WEATHER
	weather_report = await fetch(`./data/weather_data.json`)
	weather_report = await weather_report.json()

	var weatherEl = document.querySelector(".owl-stage")


	console.log(weather_report)
	weather_report.hourly.forEach((info)=> {
		let {temp} = info
		...
		let result = `
				<div class="weather-box">
					<i class="fa fa-poo-storm fa-5x"></i>
					<span class="time">10 AM</span>
					<span class="temp">${temp}<small>°c</small></span>
					</div>
				`
